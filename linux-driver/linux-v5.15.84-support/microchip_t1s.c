...
#define LAN867X_IF_TYPE			7
#define LAN867X_IF_TYPE_MASK		0x3
...
static int lan867x_revc_config_init(struct phy_device *phydev)
{
	...
	if (ret < 0)
		return ret;
	if (((ret >> LAN867X_IF_TYPE) & LAN867X_IF_TYPE_MASK) ==
	    LAN867X_RMII_IF_TYPE) {
		ret = phy_write_mmd(phydev, MDIO_MMD_VEND2,
	...
	ret = lan867x_revc_setup_cfgparam(phydev);
	if (ret < 0)
		return ret;
